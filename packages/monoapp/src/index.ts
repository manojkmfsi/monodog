...
const app = express();
    app.get('/env-config.js', (req, res) => {
      ...
      res.send(`window.ENV = { API_URL: "${`${appConfig.server.host}:${appConfig.server.port}`|| 'localhost:8999'}" };`);
    });
...
app.use((req, res, next) => {
    if (/(.ico|.js|.css|.jpg|.png|.map)$/i.test(req.path)) {
        ...
        res.sendFile('index.html', { root: path.resolve(__dirname, '..','monodog-dashboard','dist') });
    }
});
